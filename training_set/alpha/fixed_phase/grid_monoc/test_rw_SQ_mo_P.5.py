...
def run_simulation(P):
    ...
    kz_list = (np.arange(10) + 1) / 10  # ---------> k_z
    alpha_list = (np.arange(10)) / 10  # ---------> alpha
    ...
    filename = f'S_q_{P}.5_ex.mat'  # ---------> kappa
    mdict = {'S_q_kxy': S_q_kxy, 'k_grid': k_grid}
    savemat(filename, mdict)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Run simulation with given P.')
    parser.add_argument('-P', type=int, required=True, help='Value of P')
    args = parser.parse_args()

    run_simulation(args.P)